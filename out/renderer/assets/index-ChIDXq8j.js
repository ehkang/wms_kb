...
const isDev = false;
const WMS_HOST = "http://10.20.88.14:8008";
const WCS_HOST = "http://10.20.88.14:8009";
const API_CONFIG = {
  // WMS API 基础路径
  WMS_BASE_URL: isElectron ? `${WMS_HOST}/api/warehouse` : "/api/warehouse",
  // Web 开发环境：使用 Vite 代理路径
  // WCS API 基础路径
  WCS_BASE_URL: isElectron ? `${WCS_HOST}/api/WCS` : "/api/WCS",
  // Web 开发环境：使用 Vite 代理路径
  // WebSocket/SignalR Hub URL
  WS_URL: isElectron ? `${WCS_HOST}/hubs/wcsHub` : "/hubs/wcsHub",
  // Web 开发环境：使用 Vite 代理路径
  // 调试信息
  DEBUG_INFO: {
    isElectron,
...
class WMSStore {
  ...
  wmsAPI;
  wcsAPI;
  watchDeviceCodes = ["Crn2002", "TranLine3000"];
  coordinateDevices = ["Crn2001", "Crn2002", "RGV01"];
  watchStationNos = [];
  constructor() {
    this.state = reactive({
      ...
      operationMode: "InOut",
      deviceTrayMap: /* @__PURE__ */ new Map()
    });
    this.initializeAPI();
  }
  initializeAPI() {
  ...
  updateDeviceTrayMap() {
    const deviceTrayMap = /* @__PURE__ */ new Map();
    ...
    for (const [deviceCode, device] of Object.entries(this.state.devices)) {
      ...
      if (device.childrenDevice && device.childrenDevice.length > 0) {
        const hasChildrenInMonitoring = device.childrenDevice.some(
          (child) => this.watchStationNos.includes(child.code)
        );
  ...
  async handleInfo() {
    if (Object.keys(this.state.devices).length === 0) {
      ...
      this.state.deviceTrayMap = /* @__PURE__ */ new Map();
      return;
    }
    const { deviceTrayMap, operationMode } = this.updateDeviceTrayMap();
    const trayCodeMap = /* @__PURE__ */ new Map();
  ...
  async updateDevice(deviceNo, newInfo) {
  ...
  get wmsConnectionStatus() {
    return this.state.wmsConnectionStatus;
  }
  get wcsConnectionStatus() {
...
class HubConnection {
  ...
  async _startInternal() {
    ...
    try {
      let version2 = this._protocol.version;
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const addLog = (logLevel, message, categoryName = "系统") => {
    ...
    const initSignalR = async () => {
      try {
        ...
        signalRConnection = new HubConnectionBuilder().withUrl(url, {
          transport: HttpTransportType.WebSockets | HttpTransportType.LongPolling
        }).withAutomaticReconnect([0, 2e3, 1e4, 3e4]).build();
        signalRConnection.on("DeviceDataUpdate", (deviceNo, newInfo) => {
          wmsStore2.updateDevice(deviceNo, newInfo);
        });
        signalRConnection.on("logger", (logData) => {
          addLog(logData.logLevel || 2, logData.message, logData.categoryName || "系统");
        });
        signalRConnection.onreconnecting(() => {
          wmsStore2.setWcsConnectionStatus("reconnecting");
        });
        signalRConnection.onreconnected(() => {
          wmsStore2.setWcsConnectionStatus("connected");
        });
        signalRConnection.onclose(() => {
          wmsStore2.setWcsConnectionStatus("disconnected");
        });
        ...
        wmsStore2.setWcsConnectionStatus("error");
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
        ...
        createBaseVNode("header", {
          class: normalizeClass(["header", { error: errorMessage.value }])
        }, [
          createBaseVNode("h1", _hoisted_2, [
            createTextVNode(" 站台: " + toDisplayString(stationName.value) + " (" + toDisplayString(localStationNo.value) + ") ", 1),
            errorMessage.value ? (openBlock(), createElementBlock("span", _hoisted_3, " - " + toDisplayString(errorMessage.value), 1)) : createCommentVNode("", true)
          ]),
              ...
              containers.value.length === 0 ? (openBlock(), createElementBlock("div", _hoisted_21, "当前暂无托盘")) : (openBlock(true), createElementBlock(Fragment, { key: 1 }, renderList(containers.value, (tray) => {
                return openBlock(), createElementBlock("div", {
                  ...
                  class: normalizeClass(["tray-item", {
                    active: currentContainer.value === tray.code,
                    blink: currentContainer.value === tray.code && isLight.value
                  }])
                }, [
              ...
              createBaseVNode("div", _hoisted_42, [
                ...
                createBaseVNode("div", _hoisted_43, [
                  withDirectives(createBaseVNode("select", {
                    ...
                    createBaseVNode("option", { value: "error" }, "错误", -1)
                  ]), 512), [
                    [vModelSelect, logFilter.value]
                  ]),
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _cache) => {
      return openBlock(), createBlock(Dashboard);
    };
  }
});
